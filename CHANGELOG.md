...
### Changed
- The Android home-screen widget's background is now semi-transparent (~90% opacity) instead of fully opaque, for a subtle "frosted glass" look over the wallpaper — true background blur isn't possible for home-screen widgets (that's a launcher-level capability, not exposed to apps), so this is the closest achievable approximation

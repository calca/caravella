...
- The widget's 2x2 layout now shows this week's total as a small colored chip under today's total, reusing data the widget already computed but never displayed; hidden on the 1x1 size where there's no room
- The widget's "not configured" state is now tappable anywhere (previously only its small "Select group" chip responded to taps, and at compact sizes that chip isn't even shown — so a freshly placed compact widget had no working tap target at all until reconfigured at a larger size)
- **Paid-by and category pickers redesigned as horizontally scrolling chip rows**, in both the compact quick-add form and the extended/full expense form: participants and categories now show as tappable chips (last-used first), the compact form gains its own dedicated row per field (previously a single shared row), and a "+" chip opens the existing add flow inline. When there are more than 5 values, the 5th slot collapses the rest behind a "..." chip that opens a picker sheet — showing the current selection's name instead when it's one of the hidden ones. Selected chips get a soft `primary`-tinted fill with a colored border and label; idle chips use a lighter neutral fill with a thinner hairline border. In the compact form, the auto-location activity indicator now sits on the description line, pinned to the right, instead of next to the category row. Fixed the chip row painting over its own leading icon (person/category) during scroll/bounce — it inherited an unclipped `Clip.none` viewport that let chips bleed past the row's bounds
...
### Fixed
- Fixed the release APK build failing (`permission_handler_android-14.0.0: cannot find symbol ACCESS_LOCAL_NETWORK/CINNAMON_BUN`) — that plugin's 14.0.0 requires `compileSdk` 37 (its own breaking-change note), but Flutter 3.47.2's own default is still 36, and `android/build.gradle.kts` additionally forced every plugin subproject back down to 36 regardless. Bumped both to 37
- Fixed the Android home-screen widget's own configuration screen not matching the app's colors — it used a bare, un-themed `MaterialTheme` (Compose Material3's generic purple baseline) instead of the app's teal brand colors, and never picked up Material You dynamic color on Android 12+ even though the widget itself already does. The config screen now uses the same dynamic-color-with-teal-fallback theme as the rest of the widget
- Fixed the widget sometimes not updating right after saving its configuration from the widget's own "select group"/reconfigure buttons — the post-save refresh ran as an unawaited background task that could lose the race against the configure screen closing. It's now awaited before the screen closes, guaranteeing the widget reflects the new settings
- Fixed the "Gestione dati" row in Settings → Dati showing the exact same description as the section header directly above it (copy-paste duplication) — now shows what the row actually leads to ("Backup & Ripristino")
- Fixed the What's New page showing a raw, untranslated exception message to the user if the bundled changelog fails to load (and logging nothing at all in that case) — now logs via `LoggerService` and shows only the existing localized error message
- Fixed deleting a group type template failing silently (no error toast, no logging) when the underlying save fails — now handled the same way the template editor's own save failure already is
- Removed a duplicated `<supports-screens>` block in `AndroidManifest.xml` (identical element, including its comments, declared twice in a row)
- Fixed the `addExpense` Android App Function (AI agent integration) launching the app with a `groupId` that doesn't exist when no `amount` is given, instead of returning a clear "group not found" error the way `getGroupBalance`/`getRecentExpenses`/`getTodayTotal` already do — the app would open only to show its own "group not found" message, instead of the AI agent knowing immediately. Also, a `amount` of 0 or negative previously fell through silently to "amount not provided" (opens the app for manual entry) instead of being rejected with a validation error
- **The Android home-screen widget's own configuration screen (choose group) was almost entirely untranslated** — every string except 4 fell back to English regardless of the app's language. Added the missing Italian translations (title, empty state, Save button and its disabled-state message, currency label, accessibility description, widget-picker preview text) and removed string resources that turned out to be declared but never actually referenced anywhere
- The widget configuration screen's disabled Save button now also shows "Select a group to save" as visible text, not just as a screen-reader-only description — previously a sighted user saw a greyed-out button with no visible explanation
- Fixed the "Blocca schermate" (FLAG_SECURE) toggle in Settings → Privacy showing as ON even when the native call failed silently — it now only persists/announces the new state after confirming the platform actually applied it, and shows an error toast otherwise
- Fixed the "Colore dinamico" toggle (Settings → Aspetto) announcing the wrong accessibility hint to screen readers — it reused FLAG_SECURE's "double tap to enable/disable **screen security**" wording; same bug on the "App Functions" toggle in Settings → Privacy. Both now rely on their existing label + toggle-state semantics instead of a misleading hint
- Fixed restoring a backup with multiple groups: if a group failed to save partway through the list, the groups already written to disk stayed there but the UI never reflected it (no refresh, generic failure toast) — the home screen now updates for whatever was actually saved before the error is shown
- Fixed the group type template editor's Save button staying permanently disabled with no error message if saving failed — it's now wrapped in error handling like every other save flow in Settings, with a toast and the button re-enabled on failure
- Fixed several Settings error messages being hardcoded in Italian regardless of the app's language: the backup-import failure details, "couldn't open link" (Info → sviluppatore), "failed to load" (Novità), and the in-debug "Debug Logs" row title/description (previously hardcoded English with no translation key at all)
- The Backup/Restore rows in Settings → Gestione dati now use the same shared `SettingsCard` component as every other settings row, instead of a hand-rolled `Card`/`ListTile` that (among other things) exposed no accessibility semantics to screen readers
- Fixed a `TextEditingController` leak in the "change display name" dialog (Settings → Generali) — it's now disposed once the dialog closes
- Fixed the Android home-screen widget's "+" button and body tap only opening the group instead of the add-expense sheet — the native widget sent the same intent used by app shortcuts (which just navigates to the group), never the `caravella://home_widget/add_expense`/`/open_group` deep links the Flutter side already parsed. The widget now genuinely opens the add-expense sheet from the "+" button and the group page from tapping the rest of the widget
- Fixed the Android home-screen widget rendering the wrong (garbled) background color when a group's custom color is used — it converted the stored ARGB color through the wrong Compose `Color` constructor
- Fixed `ENABLE_ANDROID_WIDGET`'s Flutter-side default always being `true` regardless of flavor, which could disagree with the native per-flavor Gradle default (off for `prod`) on a build that doesn't pass the flag explicitly; both sides now default off for `prod` and on for `dev`/`staging`
- The Android home-screen widget's background image is now downscaled to roughly the widget's own size before rendering, instead of decoding a multi-megapixel group photo at full resolution on every refresh
- Fixed the Android home-screen widget refreshing itself twice on every periodic update tick (redundant storage reads)
- **Google Play flexible updates now actually get installed.** Starting an in-app update only began the background download; nothing tracked when it finished, so `completeFlexibleUpdate()` was never called and the downloaded update sat unused. The update check now listens for the download to finish (`AppUpdateNotifier`/`InAppUpdate.installUpdateListener`) and also detects one that finished while the app was closed, prompting to install it — both from Settings → What's New and from the automatic startup check
- Fixed the expense edit page's AppBar back arrow not showing the "discard changes?" warning after editing a field (e.g. the amount) — it called `Navigator.pop()` directly, which forces the pop and skips `PopScope`'s unsaved-changes guard entirely, unlike the system back button/gesture (which goes through `maybePop()` and did already show it)
...
- Fixed the Android home-screen widget's background rendering fully transparent instead of the app's surface color (white/dark) — `background()`+`cornerRadius()` on the true root element returned from `provideContent()` was unreliable; the painted surface now lives on an inner `Box` one level below the root, which only carries sizing and the click target
- The widget's `initialLayout` (the static placeholder the launcher shows before Glance's `provideGlance()` session replaces it with the real content) no longer uses `?android:attr/colorBackground` for its background — that theme attribute resolves against the widget host/launcher process's own theme, not the app's, so on hosts with an unexpected `colorBackground` it could render as an unstyled/washed-out background if the placeholder stayed visible for any length of time. Now uses a hardcoded color matching `WidgetTheme.kt`'s `WidgetSurfaceColor` (light/dark)
- Fixed the Android home-screen widget's "+" button (and body tap) doing nothing but opening the app to the home page when tapped from a cold start (app not already running), instead of showing the add-expense sheet / navigating to the group — the tap was dispatched during app startup, before `runApp()` had attached the widget tree, so the navigation code found no `BuildContext` yet and silently dropped the action. It now waits briefly for the app's navigator to become ready before dispatching

### Changed
- Increased font sizes across both Android home-screen widget layouts for legibility (group name, today/week values) — most noticeably the 1×1 layout's group name, previously 9sp
- The Android home-screen widget's 2×2 layout now splits into two zones instead of one centered block: the group name is pinned top-start (its own row), and today's total + the "week" chip are truly centered in whatever space is left below it (`ColumnScope.defaultWeight()`) — the header stays put regardless of group name length, while the hero number reads as the clear focal point instead of the whole block just sitting mid-cell. Group name and today's total are both larger (18sp/24sp, was 13sp/18sp), and the cell now has its own uniform 12dp inset on every edge — including around the CTA button, so its margin from the corner matches the title's margin from the top — instead of sharing the 1x1 layout's much tighter 6dp padding. Today's total and the "week" chip now also center in the exact gap between the title and the CTA button (reserving the button's own footprint from the centering region), rather than against the cell's true bottom edge — the button is a sibling overlay, not part of the column layout, so without that reservation the centering ignored where the button actually sits. The "week" chip's text is now genuinely vertically centered inside it — this took two passes: adding `contentAlignment = Alignment.Center` to the chip's `Box` (it previously defaulted to top-start) wasn't sufficient on its own, because RemoteViews' `TextView` (Glance's `Text` has no way to turn off `includeFontPadding`) reserves visibly more invisible leading above the glyphs than below regardless of container alignment. Measured directly off a rendered screenshot (pixel rows of chip-background vs. glyph color): equal 2dp/2dp padding still left the visible text sitting with ~10dp of space above it and only ~5dp below. Padding is now deliberately asymmetric (1dp top / 6dp bottom) to compensate, verified by the same pixel measurement landing on 9dp/9dp after the change
- The Android home-screen widget's background is now 10% transparent (90% alpha, `0xE6`) instead of fully opaque, for a subtle frosted look over the wallpaper — verified pixel-for-pixel against the expected alpha blend on a rendered screenshot (a first pass at 5%/`0xF2` was tried and replaced with this stronger value). The same `0xE6` value shipped once already earlier this cycle, back when the real bug was the background rendering fully transparent on every device (see the `ColorProvider` fix above), which made it impossible to tell intended alpha from the bug and got reverted to fully opaque as a debugging step; safe to reintroduce now that the actual cause is fixed
- The Android home-screen widget's 2×2 "+" CTA button is now a fixed 44dp square (`GlanceModifier.size(...)`, close to Android's recommended 48dp touch target) instead of padding wrapped around the "+" glyph — the glyph's own bounding box is taller than it is wide, so the old padding-only approach rendered a narrow, visibly non-square pill instead of a square button
- The Android home-screen widget now always shows the group name at every size (previously optional, and omitted by default at the smallest 1×1 size) — at 1×1 it's shown above today's total, both lines sized down to fit the cell. Removed the "show group name" toggle entirely, along with the config screen's now-empty Options tab, since the group name being visible wasn't optional in practice
- **Removed the Android home-screen widget's background image/color feature** ("Use group style as background" toggle and the transparency slider) — the widget now always renders on a plain surface matching the app's light/dark theme instead of a group photo or custom color, removing a source of visual inconsistency and the image-decoding work it required on every refresh
- **Split the Android widget's native Kotlin implementation out of one ~800-line file** (`HomeWidgetProvider.kt`) into `WidgetTheme.kt` (design tokens, dynamic-color logic), `WidgetFormatting.kt` (pure amount/color/bitmap-sizing helpers) and `HomeWidgetPrefs.kt` (per-widget config persistence), leaving `HomeWidgetProvider.kt` focused on what the widget actually renders. Also added the module's first JVM unit tests (`android/app/src/test/`, new `testImplementation("junit:junit")` dependency) covering the now-extracted pure helpers — including a regression test for the `Color(Int)` vs `Color(ULong)` bug fixed earlier this session
- **Consolidated the native (non-Flutter-engine) SQLite reads used by the Android widget and App Functions behind two SQL views** instead of five separate hand-rolled queries in `AppFunctionStorageReader.kt`: `group_summary_view` (active groups, "archived = 0" filter, today/week/total spend per group) now backs `getActiveGroupsSqlite`, `getTotalBalanceSqlite` and `getWidgetTotalsSqlite`, and a new `expense_details_view` (expenses pre-joined with category/participant names) backs `getRecentExpensesSqlite`. Same behavior, but the "which groups are active" filter and the expenses JOIN each exist in exactly one place instead of being repeated per query
- The Android home-screen widget's native default now varies by flavor: enabled on `dev` and `staging`, disabled on `prod` (`android/app/build.gradle.kts` now sets `enable_android_widget` per product flavor instead of one global default). CI's `staging` builds pass `ENABLE_ANDROID_WIDGET=true` and `prod` builds pass `=false` to match; an explicit `enableAndroidWidget` Gradle property or `ENABLE_ANDROID_WIDGET` env var still overrides the default for every flavor
- Migrated to Flutter 3.47's standalone `material_ui` package: every `package:flutter/material.dart` import in `lib/` and in the `caravella_core`, `caravella_core_ui`, and `play_store_updates` packages now imports `package:material_ui/material_ui.dart` instead (via `dart fix --apply --code=migrate_design_widgets`), with `material_ui: ^1.0.0` added to each affected `pubspec.yaml`. Since `flutter gen-l10n` doesn't generate material_ui-aware localization delegates yet, added a small hand-written `lib/l10n/app_localization_delegates.dart` (not regenerated by `gen-l10n`) providing the correct delegate list, used everywhere a `MaterialApp` needs `AppLocalizations` instead of the generated `AppLocalizations.localizationsDelegates`. Upgraded `dynamic_color` 1.8.1 → 2.1.0, which now produces a `material_ui` `ColorScheme` natively, so the temporary converter this migration originally added in `caravella_app.dart` to bridge dynamic_color's still-legacy `ColorScheme` has been removed again. No behavior change
...
- Upgraded `file_picker` 12.0.0-beta.7 → 12.1.1, migrating the two single-file pick call sites (`FilePickerServiceImpl.pickFile`, the backup/restore import in `data_backup_page.dart`) from `FilePicker.pickFiles()` + `.files.single` to the new dedicated `FilePicker.pickFile()` (returns a single `PlatformFile?` directly); also upgraded `googleapis` 16.0.0 → 17.0.0 (`google_drive_sync`), `flutter_secure_storage` 10.3.1 → 11.0.0 (`caravella_core`), `permission_handler` 12.0.3 → 13.0.1, `google_mlkit_text_recognition` 0.16.0 → 0.17.1, and a broad set of transitive dependencies across the root app and all 5 packages (no code changes needed)
- Upgraded Flutter from 3.44.0 to 3.47.2 (Dart 3.13.2), pinned in both CI workflows; `analysis_options.yaml` picked up the tool's new auto-migration excluding `build/`, `android/`, `ios/`, `web/`, `windows/`, `macos/`, `linux/` from analysis. No source changes were needed — `flutter analyze` and the full test suite (root + all 5 `packages/*`) pass unchanged

...
- **The Android home-screen widget's own configuration screen (choose group / widget options) was almost entirely untranslated** — every string except 4 fell back to English regardless of the app's language. Added the missing Italian translations (title, tab labels, empty state, both option toggles, transparency label, Save button and its disabled-state message, currency label, accessibility description, widget-picker preview text) and removed 5 string resources that turned out to be declared but never actually referenced anywhere (dead widget-picker preview values and two leftover labels from an earlier version of the tap-action design)
- Fixed the widget configuration screen's "Use group style as background"/"Show group name" checkbox rows not exposing a single accessible label to screen readers — the row's tap handling and the checkbox's own handling were two separate, unmerged accessibility nodes; now uses `Modifier.toggleable()` so TalkBack announces one coherent "label, checkbox, checked" instead of two disconnected elements
- The widget configuration screen's disabled Save button now also shows "Select a group to save" as visible text, not just as a screen-reader-only description — previously a sighted user who opened the Options tab before picking a group saw a greyed-out button with no visible explanation
...
### Changed
- The Android home-screen widget's 1×1 layout now also shows the group name (when the "show group name" option is on) above today's total, instead of just the bare total — both lines sized down to fit the smallest cell
- **Removed the Android home-screen widget's background image/color feature** ("Use group style as background" toggle and the transparency slider in the widget's Options tab) — the widget now always renders on a plain surface matching the app's light/dark theme instead of a group photo or custom color, removing a source of visual inconsistency and the image-decoding work it required on every refresh

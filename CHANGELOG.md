...
### Added
- **Android home-screen widget now targets four grid shapes explicitly — 1×1, 2×2, 4×1 and 4×2** — instead of three loosely-sized breakpoints. Each shape gets a purpose-built layout: 1×1 shows just today's total, 2×2 keeps the existing group name/today/week layout, 4×2 keeps the existing full card (today pill, group name, group total, CTA), and 4×1 is a brand-new single-row layout (group name · today total, with a small "+" chip when it's a genuinely different action from tapping the rest of the row) for the short-but-wide shape that had no dedicated design before. The native widget provider's min/resize bounds now match Android's own per-cell-count sizing guidance so the four shapes align with what launchers actually offer
...
- The widget's compact (1x1/2x2) layout now shows this week's total as a small secondary line under today's total, reusing data the widget already computed but never displayed; hidden only on the smallest 1x1 cell size where there's no room
...
### Fixed
- **Diagnostic step, not a confirmed fix**: temporarily dropped `cornerRadius()` from the Android home-screen widget's background `Box` (still fully opaque, now square corners) to isolate a report of the background rendering fully transparent on a Samsung One UI 7 device, while other apps' widgets on the same device/launcher render fine — ruling out a device-wide launcher limitation. `cornerRadius()` + `background()` (translated by Glance to `RemoteViews.setViewOutlinePreferredRadius` on API 31+) is the most non-standard modifier combo this widget uses versus a typical hand-rolled RemoteViews widget, making it the leading suspect
...
### Changed
- Increased font sizes across all four Android home-screen widget layouts for legibility (group name, today/week values, group total, "Total Spent"/"today" pill labels) — most noticeably the 1×1 layout's group name, previously 9sp
- Enlarged the Android home-screen widget's "+" CTA button — both the full-size square button (4×2 layout) and the smaller chip variant (4×1 layout) were reported as too small to comfortably tap

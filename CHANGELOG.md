...
- Migrated to Flutter 3.47's standalone `material_ui` package: every `package:flutter/material.dart` import in `lib/` and in the `caravella_core`, `caravella_core_ui`, and `play_store_updates` packages now imports `package:material_ui/material_ui.dart` instead (via `dart fix --apply --code=migrate_design_widgets`), with `material_ui: ^1.0.0` added to each affected `pubspec.yaml`. Since `flutter gen-l10n` doesn't generate material_ui-aware localization delegates yet, added a small hand-written `lib/l10n/app_localization_delegates.dart` (not regenerated by `gen-l10n`) providing the correct delegate list, used everywhere a `MaterialApp` needs `AppLocalizations` instead of the generated `AppLocalizations.localizationsDelegates`. Also added a `ColorScheme` converter in `caravella_app.dart` to bridge the still-unmigrated `dynamic_color` package's legacy `ColorScheme` into `material_ui`'s type for the dynamic-color theme. No behavior change

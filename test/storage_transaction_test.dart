...
void main() {
  group('StorageTransaction', () {
    ...
    group('Transaction Validation', () {
      ...
      test('should fail if trying to update non-existent group metadata', () async {
        final result = await repository.executeTransaction((tx) {
      ...
      test('should prevent execution of already executed transaction', () async {
    ...
    group('Data Integrity', () {
      test('should maintain data integrity across transaction', () async {
        ...
        final result = await repository.executeTransaction((tx) {
          tx.saveGroup(testGroup2.copyWith(pinned: true)); // Would create second pinned
        });
        ...
        final pinnedGroups = allGroups.data!.where((g) => g.pinned && !g.archived).toList();

...
void main() {
  ...
  group('Background removal issues - FIXED', () {
    test(
      'FIXED: Color should be removed when removeImage is called with only color set',
      () async {
        ...
        expect(
          state.color,
          isNull,
          reason: 'Color should be null after removeImage()',
        );
        expect(state.imagePath, isNull);
      },
    );
    ...
    test(
      'FIXED: Original color should not reappear when removing image in edit mode',
      () async {
        final state = GroupFormState();
        ...
        expect(
          state.color,
          isNull,
          reason: 'Original color should not reappear after image removal',
        );
    ...
    test(
      'Background removal works correctly with both image and color scenarios',
      () async {
        final state = GroupFormState();
        final controller = GroupFormController(state, GroupEditMode.create);
    ...
    test(
      'State consistency: mutual exclusion of image and color still works',
      () async {
        final state = GroupFormState();
        ...
        expect(
          state.imagePath,
          isNull,
          reason: 'Setting color should clear image',
        );

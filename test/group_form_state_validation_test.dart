...
void main() {
  group('GroupFormState validation', () {
    test('isValid should be false when title is empty', () {
      ...
      expect(state.isValid, isFalse, reason: 'Empty title should make form invalid');
    });
    ...
    test('isValid should be false when participants list is empty', () {
      ...
      expect(state.isValid, isFalse, reason: 'Empty participants should make form invalid');
    });
    ...
    test('isValid should be false when categories list is empty', () {
      ...
      expect(state.isValid, isFalse, reason: 'Empty categories should make form invalid');
    });
    ...
    test('isValid should be true when all required fields are present', () {
      ...
      expect(state.isValid, isTrue, reason: 'All required fields present should make form valid');
    });
    ...
    test('isValid should become false when last category is removed', () {
      ...
      expect(state.isValid, isFalse, reason: 'Removing last category should make form invalid');
    });

    test('isValid should remain true when one category remains after removal', () {
      final state = GroupFormState();
      state.setTitle('Test Group');
      state.addParticipant(ExpenseParticipant(name: 'John', id: 'p1'));
      state.addCategory(ExpenseCategory(name: 'Food', id: 'c1'));
      ...
      expect(state.isValid, isTrue, reason: 'At least one category should keep form valid');
    ...
    test('isValid should handle whitespace-only title', () {
      ...
      expect(state.isValid, isFalse, reason: 'Whitespace-only title should make form invalid');
    });
    ...
    test('isValid should handle title with leading/trailing whitespace', () {
      ...
      expect(state.isValid, isTrue, reason: 'Title with content should be valid even with whitespace');
    });
  });
}

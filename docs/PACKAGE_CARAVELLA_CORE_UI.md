...
| `ChipSelectorRow<T>` | Horizontally scrolling row of selectable chips for compact single-value pickers; shows up to 4 items (in whatever order the caller passes), collapses the rest behind a "..." chip (label falls back to the hidden selection's name, never a count) that opens `showSelectionBottomSheet`, plus an optional trailing "+" chip for inline add | `items`, `selected`, `itemLabel`, `onSelected`, `onAddItemInline` |

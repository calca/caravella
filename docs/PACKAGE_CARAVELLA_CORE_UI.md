...
| `showSelectionBottomSheet<T>` | Generic picker with inline "add new item" support and duplicate-name checking; `autoStartAdd` opens it straight into the inline-add text field | `items`, `selected`, `itemLabel`, `onAddItemInline`, `autoStartAdd` |
| `ChipSelectorRow<T>` | Horizontally scrolling row of selectable chips for compact single-value pickers; shows up to 4 items (in whatever order the caller passes), collapses the rest behind an icon-only "more" chip (label falls back to the hidden selection's name, never a count) that opens `showSelectionBottomSheet`, plus an optional trailing "+" chip for inline add | `items`, `selected`, `itemLabel`, `onSelected`, `moreLabel`, `onAddItemInline` |
...
- **`caravella_themes.dart`** — hand-authored Material 3 `ColorScheme` consts: `lightColorScheme` (seed primary `0xFF009688` teal) and `darkColorScheme` (a custom "soft dark" scheme tuned for WCAG AA contrast). `CaravellaThemes.createLightTheme({dynamicColorScheme})` / `createDarkTheme(...)` build full `ThemeData` (Material 3, `AppTypography.fontFamily`, custom text theme, dialog/input decoration themes), supporting Android dynamic color (Material You) with a fallback to the hardcoded scheme's container shades. Precomputed statics: `CaravellaThemes.light` / `.dark`. Also defines the `CaravellaSemanticColors` extension on `ColorScheme` — `.success`/`.onSuccess`/`.warning`/`.onWarning`, contrast-checked (≥4.5:1) against `surface` in both themes — use these instead of ad hoc `Colors.green`/`Colors.amber` for "ok/attention" status (there's no `.error` counterpart: Material 3's own `ColorScheme.error`/`.onError` already cover that role).
- **`app_text_styles.dart`** — `AppTypography.fontFamily` (`'Montserrat'`), the single source of truth for the app font family — never hardcode the string elsewhere. `AppTextStyles` convenience tokens: `sectionTitle`, `listItem`, `listItemStrong`, `subtle` (all derived from `Theme.of(context).textTheme`), plus non-theme-derived `static const TextStyle` tokens for fixed-background contexts: `mediaOverlayMessage`/`mediaOverlayCaption` (white text over the fixed black background of image/video/PDF viewers) and `emojiDisplay` (72pt, the wizard's emoji picker steps).
- **`app_dimens.dart`** — `AppDimens.minTouchTarget` (48.0) / `.minTouchTargetCompact` (44.0) / `.minTouchTargetInline` (24.0, WCAG 2.5.8 floor for targets embedded in a text flow), the shared touch-target constants. Prefer these over hardcoding `Size(48, 48)`/`44`/etc. on `styleFrom(minimumSize: ...)`.
- **`app_spacing.dart`** — `AppSpacing` padding/gap scale derived from the values already dominant in the app (`xxs`=4, `xs`=8, `sm`=12, `md`=16, `lg`=20, `xl`=24, `xxl`=32). Use these instead of hardcoding `EdgeInsets`/`SizedBox` numbers in new shared components; existing call sites are migrated incrementally (see `plan.todo.ds.md`), not all at once.
- **`app_radius.dart`** — `AppRadius` corner-radius scale (`xs`=4, `sm`=8, `md`=12, `lg`=16, `dialog`=28 — the Material 3 large-shape dialog radius, used by `Material3Dialog` and `CaravellaThemes`' dialog theme). `BaseCard`'s default radius is `AppRadius.lg`.
- **`form_theme.dart`** — `FormTheme` spacing constants (`fieldSpacing`, `sectionSpacing`, ...) aliased to `AppSpacing` so form spacing stays in sync with the rest of the scale, plus `iconSpacing` (6.0, inline icon-to-text gap fine-tuned for forms — intentionally not on the general scale) and decoration builders: `getStandardDecoration`, `getMultilineDecoration`, `getBorderlessDecoration` (all three accept an optional `labelText` alongside `hintText` — prefer it for real form fields, since a hint alone disappears once the user types, see WCAG 3.3.2), `getBorderlessAmountDecoration` (large amount entry, hint-only by design), `getSearchPillDecoration` (28dp pill search field, hint-only by design).

Always match these tokens instead of hardcoding spacing/shape/text values. `validate_design_tokens.sh` guards the highest-risk regressions (see `plan.todo.ds.md`) in CI.

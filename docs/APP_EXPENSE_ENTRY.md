...
Widget tree in `build()`: `ExpenseFormCompactHeader` ("In group: <title>") → `ExpenseFormFields` (amount, name, paidBy+category — each stacked on its own row in both compact and full/expanded mode; touched-and-invalid fields get a subtle red-tinted `AnimatedContainer`) → conditionally `ExpenseFormExtendedFields` (date, location, attachments, note — shown in full-edit mode, when editing an existing expense, or once the user expands the compact card) → `ExpenseFormActionsWidget` (save/delete/scan-receipt/voice buttons). A vertical swipe-up gesture on the compact card triggers `orchestrator.expand()`.

`ParticipantSelectorWidget`/`CategorySelectorWidget` (`widgets/`) both render as a leading icon (`IconLeadingField`) next to a `ChipSelectorRow<T>` (`caravella_core_ui`) — a horizontally scrolling row of chips, identical in compact and extended mode. The row shows up to 4 items in whatever order the caller passes in (`ExpenseFormPage`/`ExpenseEntrySheet` pass `ExpenseGroup.getParticipantsByLastUsed()`/`getCategoriesByLastUsed()`, so the most recently used participant/category surfaces first); a group with more than 5 values collapses the rest behind a trailing "more" chip (`showSelectionBottomSheet` for the full list) — icon-only (no count), showing the current selection's name instead only when the selection itself is one of the hidden ones. A trailing "+" chip opens the same sheet in inline-add mode (`autoStartAdd`) when an add callback is supplied. The compact-mode auto-location indicator lives on the name field's row instead (pinned to the right of the description, via `ExpenseFormFields._buildNameField`), not next to either chip row.
